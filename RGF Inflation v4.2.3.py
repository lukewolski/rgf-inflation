...
def lightning_fast_vcov(phi, infoVec, s):
    ...
    for alpha in range(N):
        phi_j[:, :, alpha], phi_k[:, :, alpha] = newmeshgrid(phi_dp[:, alpha])
    d1_j, d1_k = newmeshgrid(d1)
    d2_j, d2_k = newmeshgrid(d2)
    ...
    vcov += noise*np.identity(len(vcov))
    return vcov, M


def lightning_fast_beefupdate(s, phiPoints, infoVec, beefCov, beefM, bar, A, forgotten, forgets):

    """
    Updates covariance matrix (without recomputing the whole thing) and "M", in a mostly vectorized manner.
    ...
    - beefM: [(bar + M)^2 matrix] large matrix of zeros with old covariance matrix in upper left (bar)^2 section
    - bar: [int] size of old covariance matrix
    - A:[B x C array] compression matrix
    - forgotten: [int] difference in number of points between uncompressed and compressed covariance matrix
    - forgets: [int] number of past forgetting steps
    ...
    newpoints = T-bar
    bar1 = bar+forgotten
    phi_dp = phi[infoVec[0,:T1]]

    phi_j = np.zeros((T1, T1, N))
    phi_k = np.zeros((T1, T1, N))
    new_j = np.zeros((T1,newpoints,N))
    new_k = np.zeros((T1,newpoints,N))
    for alpha in range(N):
        phi_j[:, :, alpha], phi_k[:, :, alpha] = newmeshgrid(phi_dp[:, alpha])
        new_j[:,:,alpha], new_k[:,:,alpha] = phi_j[:, -newpoints:, alpha], phi_k[:, -newpoints:, alpha]
    d1_j, d1_k = newmeshgrid(d1)
    d2_j, d2_k = newmeshgrid(d2)
    d1_j, d1_k, d2_j, d2_k = d1_j[:,-newpoints:], d1_k[:,-newpoints:], d2_j[:,-newpoints:], d2_k[:,-newpoints:]
    phi_j, phi_k = new_j, new_k

    J, K = totalderiv(T1,newpoints,N,d1_j,d1_k,d2_j,d2_k)

    n = J + K
    beefedCov = vv_covariance(phi_j,phi_k,s) * (-1) ** np.sum(n, axis=-1) * (-1) ** np.sum(K, axis=-1)
    beefedM = vv_covariance(phi_j,phi_k,s*np.sqrt(2)) * (-1) ** np.sum(n, axis=-1) * (-1) ** np.sum(K, axis=-1)

    for alpha in range(N):

        for nn in range(5):
            nn_mask = (n[:,:, alpha] == nn)
            beefedCov[nn_mask] *= He(nn, (phi_j[nn_mask,alpha] - phi_k[nn_mask,alpha]) / s ** 2)
            beefedM[nn_mask] *= He(nn, (phi_j[nn_mask,alpha] - phi_k[nn_mask,alpha]) / (s*np.sqrt(2)) ** 2)

    # Compress cross-elements of M and vcov
    if forgets > 0:
        PF = A@beefedCov[:bar1]
        MPF = A@beefedM[:bar1]
    else:
    ...
    # Update A to be useable in next update step
    if forgets > 0:
        A_1,A_2 = np.shape(A)
        beefedA = np.zeros((A_1+newpoints,A_2+newpoints))
        beefedA[:A_1,:A_2] = A
        beefedA[A_1:,A_2:] = np.identity(newpoints)
    else:
        beefedA = None

    # Add compressed elements onto vcov and M
    beefCov[bar:T,bar:T] = FF
    beefCov[:bar,bar:T] = PF
    beefCov[bar:T,:bar] = PF.T
    beefM[bar:T, bar:T] = MFF
    beefM[:bar, bar:T] = MPF
    beefM[bar:T, :bar] = MPF.T
    beefCov[bar:T,bar:T] += noise*np.identity(T-bar)

    return beefCov, beefM, beefedA

@jit
def newmeshgrid(arr):

    """
    Equivalent to np.meshgrid rewritten to be compiled with Numba.
...
@jit
def totalderiv(T1,newpoints,N,d1_j,d1_k,d2_j,d2_k):

    """
    Creation of J, K rewritten to be compiled with Numba
    """

    J = np.zeros((T1, newpoints, N))
    K = np.zeros((T1, newpoints, N))
    for alpha in range(N):
        for i in range(T1):
            for j in range(newpoints):
                if d1_j[i,j] == alpha:
                    J[i,j,alpha] = 1
                else:
                    J[i,j,alpha] = 0
...
def updateCholesky(beefCov_L, vcov, bar):
    ...
    l = sp.linalg.cholesky((toChol), lower = True)
...
class BStuff():
    ...
    def __init__(self, **kwargs):
        ...
        self.forgotten = 0 if kwargs.get('forgotten') is None else kwargs.get('forgotten')
...
def sampleVData(b, phiPoint, d1=(-1,), d2=(-1,), include=True):
    ...
    bar = len(vcov)
    beefCov, beefM, A = lightning_fast_beefupdate(s, phiList, infoVec, beefCov, beefM, bar, b.A, b.forgotten, b.forgets)
    vcov = beefCov[:len(infoVec[0])-b.forgotten, :len(infoVec[0])-b.forgotten]
        # error source may be here - very slightly negative eigenvalues of vcov right before crash
    M = beefM[:len(infoVec[0])-b.forgotten, :len(infoVec[0])-b.forgotten]
    ...
    if include:
        ...
        b.vcov_L = b.beefCov_L[:len(infoVec[0])-b.forgotten, :len(infoVec[0])-b.forgotten]
        b.nfev = b.nfev + 1
        b.M = M
        b.beefM = beefM
        b.A = A
...
def forceMin(s, V0, N):
    ...
    if N > 1:
        ...
        bar = len(b.vcov)
        b.beefCov, b.beefM, b.A = lightning_fast_beefupdate(s, b.phiStuff[0], b.infoVec, b.beefCov, b.beefM, bar, b.A, b.forgotten, b.forgets)
...
def forgetPrincipled(b):
    ...
    M = b.M
    seedNum = b.N ** 2 + b.N + 1
    CC = vcov[seedNum:,seedNum:]
    eigvals, eigvecs = sp.linalg.eigh(M[seedNum:,seedNum:], CC+100000*noise*np.identity(len(CC)))
    minval = 0.99999999*np.sum(eigvals)
    ...
    numvals = 1
    Avals = eigvals[0]
    while np.sum(Avals)<minval and numvals<len(eigvals):
        A = np.vstack((A,eigvecs[numvals]))
        Avals += eigvals[numvals]
        numvals += 1
    A = (sp.linalg.orth(A.T)).T
    A = np.block([[np.identity(seedNum),np.zeros((seedNum,len(A.T)))],[np.zeros((len(A),seedNum)),A]])
    newvcov = A@(vcov+10*noise*np.identity(len(vcov)))@A.T
    newvcov += 100*noise*np.identity(len(newvcov))
    newM = A@M@A.T
    b.vcov = newvcov
    b.M = newM
    b.vcov_L = sp.linalg.cholesky((b.vcov),lower=True)
